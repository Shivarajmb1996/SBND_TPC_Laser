...
class ComSerial(Base):
    ...
    def com_init(self, useconfig=True):
        ...
        if useconfig is True:
            print(" Hello here at comserial 40")
            self.com_load("com_ports.json")
            port = str(self.ComPortsDict[self.name])
        else:
            print(" Hello here at comserial 41")
            port = self.comport

        if self.comDryRun is False:
            print(" Hello here at comserial 42")
            try:
                print(" Hello here at comserial 50", port)
    ...
    def com_write(self, message, echo=False):
        ...
        if self.comDryRun is False:
            ...
            self.com.isOpen()
            print("_____comserial.py _____________85", msg.encode())
            print(self.com.isOpen())
            self.com.write(msg.encode())   #write(msg.encode())
