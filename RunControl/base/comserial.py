...
class ComSerial(Base):
    def __init__(self, name="", RunNumber=0, logit=True):  # , comport):
        ...
        self.comTimeout = 0.1 # it was 0.1
        ...
        self.comPrefix = "b"  # this string is put in front of any message transmitted
    ...
    def com_init(self, useconfig=True):
        ...
        if self.comDryRun is False:
            try:
                print(" ComSerial 45_____________'com_init'")
                #self.com = serial.Serial(port, self.comBaudrate, 8, 'N', 1, timeout=0.1)  #for python2
                self.com = serial.Serial(port, int(self.comBaudrate), serial.EIGHTBITS, serial.PARITY_NONE, serial.STOPBITS_ONE, timeout = 0.1)
    ...
    def com_write(self, message, echo=False):
        ...
        print("________self is _______", self)
        print("____comserial___ 76", message)

        if self.comDryRun is False:
            msg = self.comPrefix + message + self.comEnd
            self.com.isOpen()
            print("_____comserial.py _____###________80", msg.encode('ascii')," and type is ", type(msg.encode('ascii')))
            print("______comserial 84_____ The Final msg____size______", sys.getsizeof(msg.encode()))
...
#            print("_________comserial 89__ Total time of communication:", elapsed_time)
    ...
    def com_recv(self, msg_length=100):
        ...
        if self.comDryRun is False:
            try:
                ...
                sys.exit(1)
            print("____comserial.py______ 108", self.reply_filter(msg))
